...
public class UICheckbox : MonoBehaviour
{
	...
	void Set (bool state)
	{
		...
		else if (mChecked != state)
		{
			...
			if(broadcastMessage == true && !string.IsNullOrEmpty(functionName))
			{
				Debug.Log("ssssssssssssss : " + functionName);

				NGUITools.Broadcast(functionName, mChecked);
			}
			else if(eventReceiver != null && !string.IsNullOrEmpty(functionName)) // Send out the event notification
			{
				current = this;
