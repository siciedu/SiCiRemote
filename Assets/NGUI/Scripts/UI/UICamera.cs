...
public class UICamera : MonoBehaviour
{
	...
	public class MouseOrTouch
	{
		...
		public int		  pressedCount;	// kjh 추가 누른 상태가 아니면 0 1프레임당 값이 1씩 증가 한다. 뗀순간 -1 이 들어옴
	...
	// Mouse events
	static public MouseOrTouch[] mMouse = new MouseOrTouch[] { new MouseOrTouch(), new MouseOrTouch(), new MouseOrTouch() };
	...
	static public Dictionary<int, MouseOrTouch> mTouches = new Dictionary<int, MouseOrTouch>();
	...
	static public GameObject hoveredObject2
	{
		get
		{
			foreach(MouseOrTouch m in mMouse)
			{
				if(m.current != null)
					return m.current;
			}
