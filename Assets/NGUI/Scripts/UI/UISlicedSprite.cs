...
public class UISlicedSprite : UISprite
{
	...
	override public Vector2 pivotOffset
	{
		get
		{
			...
			if(p == Pivot.Top || p == Pivot.Center || p == Pivot.Bottom)
				v.x = -0.5f;
			else if(p == Pivot.TopRight || p == Pivot.Right || p == Pivot.BottomRight)
				v.x = -1f;

			if(p == Pivot.Left || p == Pivot.Center || p == Pivot.Right)
				v.y = 0.5f;
			else if(p == Pivot.BottomLeft || p == Pivot.Bottom || p == Pivot.BottomRight)
				v.y = 1f;
